...
EMMC_IMAGE = "${IMGDEPLOYDIR}/${IMAGE_NAME}.emmc"
...
IMAGE_CMD:emmcimg () {
    ...
    dd if=/dev/zero of=${UNPACKDIR}/boot.img bs=1024 count=${BOOT_PARTITION_SIZE}
    mkfs.msdos -n "${BOOTDD_VOLUME_ID}" -S 512 ${UNPACKDIR}/boot.img
    echo "setenv STARTUP \"boot emmcflash0.kernel1 'root=/dev/mmcblk1p3 rootfstype=ext4 rootwait'\"" > ${UNPACKDIR}/STARTUP
    echo "setenv STARTUP \"boot emmcflash0.kernel1 'root=/dev/mmcblk1p3 rootfstype=ext4 rootwait'\"" > ${UNPACKDIR}/STARTUP_1
    echo "setenv STARTUP \"boot emmcflash0.kernel2 'root=/dev/mmcblk1p5 rootfstype=ext4 rootwait'\"" > ${UNPACKDIR}/STARTUP_2
    echo "setenv STARTUP \"boot emmcflash0.kernel3 'root=/dev/mmcblk1p7 rootfstype=ext4 rootwait'\"" > ${UNPACKDIR}/STARTUP_3
    echo "setenv STARTUP \"boot emmcflash0.kernel4 'root=/dev/mmcblk1p9 rootfstype=ext4 rootwait'\"" > ${UNPACKDIR}/STARTUP_4
    mcopy -i ${UNPACKDIR}/boot.img -v ${UNPACKDIR}/STARTUP ::
    mcopy -i ${UNPACKDIR}/boot.img -v ${UNPACKDIR}/STARTUP_1 ::
    mcopy -i ${UNPACKDIR}/boot.img -v ${UNPACKDIR}/STARTUP_2 ::
    mcopy -i ${UNPACKDIR}/boot.img -v ${UNPACKDIR}/STARTUP_3 ::
    mcopy -i ${UNPACKDIR}/boot.img -v ${UNPACKDIR}/STARTUP_4 ::
    dd conv=notrunc if=${UNPACKDIR}/boot.img of=${EMMC_IMAGE} seek=1 bs=$(expr ${IMAGE_ROOTFS_ALIGNMENT} \* 1024)
