...
IMAGE_CMD:hdemmc () {
    ...
    echo "boot emmcflash0.kernel1 'root=/dev/mmcblk0p3 rw rootwait ${MACHINE}_4.boxmode=12'" > ${UNPACKDIR}/STARTUP
    echo "boot emmcflash0.kernel1 'root=/dev/mmcblk0p3 rw rootwait ${MACHINE}_4.boxmode=1'" > ${UNPACKDIR}/STARTUP_1
    mcopy -i ${UNPACKDIR}/boot.img -v ${UNPACKDIR}/STARTUP ::
    mcopy -i ${UNPACKDIR}/boot.img -v ${UNPACKDIR}/STARTUP_1 ::
    dd conv=notrunc if=${UNPACKDIR}/boot.img of=${EMMC_IMAGE} bs=1024 seek=${BOOT_PARTITION_OFFSET}
    dd conv=notrunc if=${DEPLOY_DIR_IMAGE}/zImage of=${EMMC_IMAGE} bs=1024 seek=${KERNEL_PARTITION_OFFSET}
    dd if=${IMGDEPLOYDIR}/${IMAGE_NAME}.ext4 of=${EMMC_IMAGE} bs=1024 seek=${ROOTFS_PARTITION_OFFSET}
}
